...
namespace ConsoleApp1
{
    internal class Program
    {
        ...
        {
            int valeurSecrete, valuerSaisie = -1, nbTentative = 0, meilleurScore = 0;
            string reponse;
            ...
            do
            {
                ...
                historiquee[Partie.getNbParties()] = new Partie(valeurSecrete);
                Console.WriteLine(historiquee[Partie.getNbParties()]);
                do
                {
                    try
                    {
                ...
                historiquee[Partie.getNbParties()].tentative = nbTentative;
                Console.WriteLine(historiquee[Partie.getNbParties()]);
            ...
            if (p == null)
            {
               ...
               " stocker votre historique ou taper Entrée pour l'afficher ici.");
                if (reponse != "")
                    historique.afficheHistorique(Partie.getNbParties() - 1, reponse);
                else
                    historique.afficheHistorique(Partie.getNbParties() - 1);
            }
            else
            {
                ...
                nomFichier += ".txt";
                historique.afficheHistorique(Partie.getNbParties() - 1, nomFichier);
            }
